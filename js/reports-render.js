...
import {
  formatCurrency, esc, AR_MONTHS, lastNMonths
} from './utils.js';

export function renderPLStatement(revenue, cogs, grossProfit, expenses, totalExpenses, netProfit) {
  const byCat = {};
...
export function renderTopProducts(sales) {
  const productSales = {};
  sales.forEach(s => {
...
export function renderExpensesByCategory(expenses) {
  const byCat = {};
  expenses.forEach(e => { byCat[e.category||'أخرى'] = (byCat[e.category||'أخرى']||0) + (e.amount||0); });
...
export function renderMonthlyTable(allSales, allPurchases, allExpenses) {
...
export function renderReportCharts(allSales, allPurchases, allExpenses) {
    ...
    new Chart(ctx1, {
      ...
      data: {
        ...
        datasets: [
          ...
          { label: 'المصاريف',   data: monthly.map(m=>m.expenses), backgroundColor: 'rgba(198,40,40,.65)',  borderRadius: 5 },
  ...
  if (ctx2) {
    new Chart(ctx2, {
      ...
      data: {
        labels: monthly.map(m => m.label),
        datasets: [
          { label: 'الربح الإجمالي', data: monthly.map(m=>m.profit),   borderColor: '#2E7D32', backgroundColor: 'rgba(46,125,50,.1)',  borderWidth: 2, fill: true,  tension: 0.4 },
          { label: 'صافي الربح',     data: monthly.map(m=>m.net),      borderColor: '#1565C0', backgroundColor: 'rgba(21,101,192,.1)', borderWidth: 2, fill: true,  tension: 0.4 },
          { label: 'المصاريف',       data: monthly.map(m=>m.expenses), borderColor: '#C62828', borderDash: [5,5],                       borderWidth: 2, fill: false, tension: 0.4 }
        ]
      },
      options: {
        responsive: true, maintainAspectRatio: false,
...
export function printReport(from, to) {
